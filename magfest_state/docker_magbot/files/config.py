...
CORE_PLUGINS = ({% for plugin in salt['pillar.get']('magbot:core_plugins', []) %}
    '{{ plugin }}',{% endfor %}
)
...
BOT_LOG_FILE = None  # We forward docker logs to syslog
...
BOT_IDENTITY = {
    # XMPP (Jabber) mode
    'username': 'magbot',  # The JID of the user you have created for the bot
    'password': '{{ salt["pillar.get"]("magbot:auth_admin", "") }}',  # The corresponding password for this user
    ...
    'token': '{{ salt["pillar.get"]("magbot:slack_token", "") }}',
...
BOT_ADMINS = ({% for admin in salt['pillar.get']('magbot:admins', []) %}
    '{{ admin }}',{% endfor %}
)
